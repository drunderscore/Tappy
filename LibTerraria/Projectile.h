...
public:
    ...
    u8 owner() const { return m_owner; }

    void set_owner(u8 value) { m_owner = value; }
    ...
    const Optional<u16>& banner_id_to_respond_to() const { return m_banner_id_to_respond_to; }

    const Optional<i16>& damage() const { return m_damage; }

    Optional<i16>& damage() { return m_damage; }

    const Optional<float>& knockback() const { return m_knockback; }

    const Optional<i16>& original_damage() const { return m_original_damage; }

    const Optional<i16>& uuid() const { return m_uuid; }

    Optional<u16>& banner_id_to_respond_to() { return m_banner_id_to_respond_to; }

    Optional<float>& knockback() { return m_knockback; }

    Optional<i16>& original_damage() { return m_original_damage; }

    Optional<i16>& uuid() { return m_uuid; }
